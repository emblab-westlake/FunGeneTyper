...
class TransformerLayer(nn.Module):
    ...
    def _init_submodules(self, add_bias_kv, use_esm1b_layer_norm):
        ...
        self.finetune_layer_norm = BertLayerNorm(self.embed_dim)
        self.finetune_adapter = nn.Sequential(OrderedDict([
            ("DownProject",nn.Linear(self.embed_dim, 256)),
            ("relu",nn.ReLU()),
            ("dropout",nn.Dropout(0.2)),
            ("UpProject", nn.Linear(256, self.embed_dim))
        ]))
    ...
    def forward(self, x, self_attn_mask=None, self_attn_padding_mask=None, need_head_weights=False):
        ...
        # Adapter
        residual_last = x
        x = self.finetune_layer_norm(x)
        x = self.finetune_adapter(x)
        x = residual_last + x
